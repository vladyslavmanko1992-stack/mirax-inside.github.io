...
import { ProjectCategory, ProjectCore } from '../types';
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      files.forEach((file, index) => {
        ...
        if (totalSize > 10000000) { // Total limit 10MB
          setError('Total image size too large (max 10MB)');
          return;
        }

        if (file.size > 2000000) { // Individual limit 2MB
  ...
  if (user.bannedUntil && new Date(user.bannedUntil) > new Date()) {
    return (
      <div className="container mx-auto p-4 max-w-2xl">
        <div className="bg-[#2c2c2c] border-4 border-[#aa0000] p-6 shadow-lg text-center">
          <h1 className="text-3xl font-vt323 text-[#aa0000] mb-4">Account Banned</h1>
          <p className="text-gray-300 font-vt323 text-lg mb-2">
            You are banned until {new Date(user.bannedUntil).toLocaleString()}.
          </p>
          {user.banReason && (
            <p className="text-gray-400 font-vt323">Reason: {user.banReason}</p>
          )}
          <p className="text-gray-500 font-vt323 mt-4">You cannot submit projects while banned.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4 max-w-2xl">
      <div className="bg-[#2c2c2c] border-4 border-[#181818] p-6 shadow-lg">
        <h1 className="text-3xl font-vt323 text-[#55ff55] mb-6 border-b border-[#444] pb-2">Submit Project</h1>
